...
import static by.legan.spacegame.framework.Input.TouchEvent.*;
import static by.legan.spacegame.framework.gl.Animation.*;
...
public class GameScreen extends GLScreen {
	...
	private void updateRunning(float deltaTime) {
		...
		for (int i = 0; i < len; i++) {
			TouchEvent event = events.get(i);
			if (event.type != TOUCH_DOWN)continue;
			guiCam.touchToWorld(touchPoint.set(event.x, event.y));
...
/*			if (OverlapTester.pointInRectangle(shotBounds, touchPoint)) {
	...
	private float calculateInputAcceleration() {
		...
		if (Settings.touchEnabled) {
			...
			if (game.getInput().isTouchDown(0)) {
				guiCam.touchToWorld(touchPoint.set(game.getInput().getTouchX(0), game.getInput().getTouchY(0)));
				if (OverlapTester.pointInRectangle(shotBounds, touchPoint)) {
					world.shoot();
				}
			}
	...
	Texture frame = Assets.sunAnim.getKeyFrame(tTime, ANIMATION_LOOPING);
	tTime += deltaTime;
//	Log.d("TimeAnim", "tTime: " + tTime);


	batcher.beginBatch(frame);
	batcher.drawSprite(240, 160, 480, 320, new TextureRegion(frame, 0,0, 512, 512));
	batcher.endBatch();

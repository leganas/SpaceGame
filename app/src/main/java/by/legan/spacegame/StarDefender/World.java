...
public class World {
	...
	private void generateAsteroid() {
		...
		int x,y,z;
		for (int i = 0; i < 8; i++) {
			x = min + rnd.nextInt((max+15) - (min+15) + 1);
			y = min + rnd.nextInt(max - min + 1);
			z = min + rnd.nextInt(max - min + 1);
			asteroids.add(new Asteroid(x, -y-25, -z-20));
		}
	}
	...
	private void checkShotCollisions() {
		...
		if (OverlapTester.overlapSpheres(invader.bounds,shot.bounds)
